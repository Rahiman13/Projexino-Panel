...
const Blogs = () => {
  ...
  const handleEdit = (blog) => {
    ...
    // Parse seoMetadata if it's a string
    let parsedSeoMetadata;
    try {
      parsedSeoMetadata = typeof blog.seoMetadata === 'string' 
        ? JSON.parse(blog.seoMetadata)
        : blog.seoMetadata || {};
    } catch (error) {
      console.error('Error parsing SEO metadata:', error);
      parsedSeoMetadata = {
        metaTitle: '',
        metaDescription: '',
        keywords: []
    ...
    // Split any comma-separated keywords
    const processedKeywords = Array.isArray(parsedSeoMetadata.keywords)
      ? parsedSeoMetadata.keywords.flatMap(keyword =>
    ...
    setFormData({
      ...
      currentKeyword: '',
      seoMetadata: {
        metaTitle: parsedSeoMetadata.metaTitle || '',
        metaDescription: parsedSeoMetadata.metaDescription || '',
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Add all text fields except special handling ones
      ...
      // Format tags and SEO metadata
      formDataToSend.append('tags', JSON.stringify(
        formData.tags.map(tag => tag.trim()).filter(Boolean)
      ));

      // Stringify the entire seoMetadata object
      const seoMetadataString = JSON.stringify({
        metaTitle: formData.seoMetadata.metaTitle || '',
        metaDescription: formData.seoMetadata.metaDescription || '',
        keywords: Array.isArray(formData.seoMetadata.keywords) 
          ? formData.seoMetadata.keywords.map(k => k.trim()).filter(Boolean)
          : []
      });
      
      formDataToSend.append('seoMetadata', seoMetadataString);
