...
const Newsletter = () => {
  ...
  const [blogForm, setBlogForm] = useState({
    subject: '',
    additionalContent: '',
    scheduledFor: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().slice(0, 16) // Default to tomorrow
  });
    ...
    try {
      const response = await fetch(`${baseUrl}/api/newsletters/schedule-blog`, {
        ...
        body: JSON.stringify({
          ...
          scheduledFor: new Date(blogForm.scheduledFor).toISOString(),
      ...
      >
        ...
        <DialogContent>
          {!newsletterType ? (
            ...
            <Box sx={{ p: 2, color: isDarkMode ? '#fff' : 'inherit' }}>
              <Typography>Blog Newsletter Form (Coming Soon)</Typography>
            </Box>
          )}
        </DialogContent>
        <DialogActions sx={{ p: 2, borderTop: 1, borderColor: isDarkMode ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)' }}>
          <Button
            onClick={() => setOpenDialog(false)}
            sx={{ color: isDarkMode ? '#fff' : 'inherit', background: isDarkMode ? 'rgba(255,255,255,0.05)' : 'white' }}
          >
            Cancel
          </Button>
          {newsletterType && (
            <LoadingButton
              ...
              loading={sending}
              disabled={!announcementForm.title || !announcementForm.subject || !announcementForm.announcement}
              sx={{
